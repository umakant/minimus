...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=`+environment.APPID)
      .subscribe((data) => {
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=`+environment.APPID)
      .subscribe((data) => {
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=`+environment.APPID)
      .subscribe((weather: any) => {
